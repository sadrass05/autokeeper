        ...
        dataLines.forEachIndexed { index, line ->
            runCatching {
                ...
                val record = ExpenseRecord(
                    merchant = cols[1].trim().replace("\"", ""),
                    amount = amount,
                    platform = cols[3].trim().replace("\"", ""),
                    paymentChannel = cols[4].trim().replace("\"", ""),
                    category = cols[5].trim().replace("\"", ""),
                    isFinanceExpense = cols[6].trim().replace("\"", "").contains("是"),
